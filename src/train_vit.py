...
def main():
    ...
    model = create_vit_model(cfg["models"]["vit_name"], cfg["models"]["num_classes"], pretrained=True).to(device)
    ...
    torch.save(model.state_dict(), Path(cfg["outputs"]["model_dir"]) / "vit_transformer.pth")
    plot_confusion_matrix(y_true, y_pred, title="Vision Transformer Confusion Matrix",
                          save_path=Path(cfg["outputs"]["result_dir"]) / "vit_confusion_matrix.png")
    plot_training_curves(history, title="Vision Transformer",
                         save_path=str(Path(cfg["outputs"]["result_dir"]) / "vit_curves.png"))
